...
const CesiumHeatmap =  new Object();
CesiumHeatmap.defaults = {
 ...
 */
CesiumHeatmap.create = (cesium, bb, options)=> {
 ...
 *  bb: the WGS84 bounding box like {north, east, south, west}
 */
CesiumHeatmap.wgs84ToMercatorBB = (bb) => {
 ...
 */
CesiumHeatmap.mercatorToWgs84 = (p) => {
 ...
 * 销毁
 */
CHInstance.prototype.destroy = function () {
	if(this._container){
	...
	if (CesiumHeatmap.defaults.useEntitiesIfAvailable && this._cesium.entities) {
		...
		this._layer = this._cesium.entities.add({
			...
			rectangle: {
				...
				material: material,
				distanceDisplayCondition: new Cesium.DistanceDisplayCondition(this._options.near, this._options.far),
			}
		});

		// var heading = Cesium.Math.toRadians(360);
		// var pitch = Cesium.Math.toRadians(-45);
		// this._cesium.flyTo(this._layer, {
        //            duration: 5,
        //            maximumHeight: 500000,
        //            offset: new Cesium.HeadingPitchRange(heading, pitch, 50000)
        //            });
	} else {
		if (this._layer) {
